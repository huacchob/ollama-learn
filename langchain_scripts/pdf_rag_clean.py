...
def ingest_pdf(doc_path: str) -> list[Document] | None:
    ...
    Returns:
        list[Document] | None: List of documents if successful, None otherwise.
    """
    if not os.path.exists(path=doc_path):
        logging.error(msg=f"PDF file not found at path: {doc_path}")
        return None
    loader: UnstructuredPDFLoader = UnstructuredPDFLoader(
        file_path=doc_path,
    )
...
def initialize_model(model: str) -> ChatOllama:
...
def main() -> None:
    ...
    # # Initialize the language model
    # llm: ChatOllama = initialize_model(model=MODEL_NAME)
    ...
    # chain: Runnable[str, str] = create_chain(
    #     retriever=retriever,
    #     llm=llm,
    # )
